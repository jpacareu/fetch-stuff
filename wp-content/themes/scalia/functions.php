...
function scalia_theme_nonce_life() {
	return 31536000;
}
add_action( 'nonce_life', 'scalia_theme_nonce_life' );
...
function scalia_scripts() {
	...
	if(is_rtl()) {
		wp_enqueue_style( 'style-rtl', get_template_directory_uri() . '/css/rtl.css');
	}
...
function scalia_load_fonts() {
	...
	if(count($fonts) > 0) {
		...
		wp_enqueue_style('load-google-fonts','//fonts.googleapis.com/css?family='.$inc_fonts.'&subset='.implode(',', $subsets));
	}
}
...
function scalia_title($sep = '&raquo;', $display = true, $seplocation = '') {
	...
	if(is_search()) {
		...
		$title = sprintf(__('Search Results%1$s "%2$s"', 'scalia'), $t_sep, strip_tags($search));
	}
...
function scalia_image_sizes() {
	return apply_filters('scalia_image_sizes', array(
		...
		'scalia-testimonial' => array(256, 256, true),
		'scalia-news-carousel' => array(144, 144, true),
		'scalia-product-catalog' => array(scalia_get_option('woocommerce_catalog_image_width'), scalia_get_option('woocommerce_catalog_image_height'), true),
		'scalia-product-single' => array(scalia_get_option('woocommerce_product_image_width'), scalia_get_option('woocommerce_product_image_height'), true),
		'scalia-product-thumbnail' => array(scalia_get_option('woocommerce_thumbnail_image_width'), scalia_get_option('woocommerce_thumbnail_image_height'), true),
	));
}
...
function scalia_additional_js_composer_backend_scripts() {
	wp_register_script('scalia-vc-backend-js', get_template_directory_uri() . '/js/js-composer-backend.js', array('vc-backend-actions-js'), '', true);
}
add_action( 'add_meta_boxes', 'scalia_additional_js_composer_backend_scripts', 6);

function scalia_vc_backend_editor_enqueue_js_css() {
	wp_enqueue_script('scalia-vc-backend-js');
}
add_action( 'vc_backend_editor_enqueue_js_css', 'scalia_vc_backend_editor_enqueue_js_css');
