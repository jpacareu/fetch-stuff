...
function scalia_woocommerce_single_product_gallery() {
	...
	foreach($attachments_ids as $attachments_id) {
		if(scalia_get_option('woocommerce_activate_images_sizes')) {
			$thumb_image_url = scalia_generate_thumbnail_src($attachments_id, 'scalia-product-thumbnail');
			$preview_image_url = scalia_generate_thumbnail_src($attachments_id, 'scalia-product-single');
		} else {
...
function scalia_wc_add_to_cart_message($message, $products) {
	$titles = array();
	$count  = 0;
...
add_filter('wc_add_to_cart_message_html', 'scalia_wc_add_to_cart_message', 10, 2);
...
function scalia_single_product_archive_thumbnail_size($size) {
	if(scalia_get_option('woocommerce_activate_images_sizes')) {
		return 'scalia-product-catalog';
	}
	return $size;
}
add_filter( 'single_product_archive_thumbnail_size', 'scalia_single_product_archive_thumbnail_size' );

function scalia_woocommerce_get_image_size_thumbnail($size) {
	if(scalia_get_option('woocommerce_activate_images_sizes')) {
		return array(
			'width' => scalia_get_option('woocommerce_thumbnail_image_width'),
			'height' => scalia_get_option('woocommerce_thumbnail_image_height'),
			'crop' => 1,
		);
	}
	return $size;
}
add_filter( 'woocommerce_get_image_size_thumbnail', 'scalia_woocommerce_get_image_size_thumbnail' );