...
function scalia_add_shortcodes_generator_metabox() {
	add_meta_box('scalia_shortcodes_generator', __('Shortcodes Generator', 'scalia'), 'scalia_shortcodes_generator_metabox', 'post', 'side', 'high');
	add_meta_box('scalia_shortcodes_generator', __('Shortcodes Generator', 'scalia'), 'scalia_shortcodes_generator_metabox', 'page', 'side', 'high');
}
add_action('add_meta_boxes', 'scalia_add_shortcodes_generator_metabox');

function scalia_shortcodes_generator_metabox($post) {
	$shortcodes = scalia_shortcodes();
?>
<div id="shortcode-generator">
	<div class="shortcodes-select">
		<label><?php _e('Select Shortcode', 'scalia') ?>:</label>
		<select>
			<option><?php _e('-- Select Shortcode --', 'scalia'); ?></option>
	...
	<div class="shortcodes-blocks">
		<?php foreach($shortcodes as $id => $shortcode) : ?>
			<div class="shortcode-item" data-name="<?php echo esc_attr($id); ?>" id="shortcode-item-<?php echo $id; ?>"<?php echo scalia_sc_is_container($shortcode) ? ' data-is_container="1"' : ''; ?>>
				<hr>
				...
				<div class="shortcode-item-params">
					<?php foreach($shortcode['params'] as $param) : ?>
						<div class="shortcode-item-param">
							...
							<div class="shortcode-item-param-field"><?php echo scalia_scg_field($param); ?></div>
	...
	<div class="shortcode-insert-button"><button><?php _e('Insert Shortcode', 'scalia'); ?></button></div>
	<div class="shortcode-result"><textarea></textarea></div>
</div>
<?php
}

function scalia_scg_field($param) {
	$output = '';
...
function scalia_sc_is_container($shortcode) {
